 ...
 * 2) see #1.
 */
public abstract class AbstractAsyncSuCMD extends AsyncTask<String, Void, String> {
     ...
     */
    public AbstractAsyncSuCMD(boolean mountSystem) {
        ...
        try {
            ...
            for (int i = 0; params.length > i; i++) {
                ...
                if (params[i] != null && !params[i].trim().equals("")) {
                    stdout = CMD.runSuCommand(params[i]).getStdout();
                } else {
